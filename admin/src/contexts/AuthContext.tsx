...
interface User { id: string; email: string; name: string; role: string; status: string; }
interface AuthCtx {
  user: User | null;
  token: string | null;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchMe = useCallback(async (t: string) => {
    try {
      ...
      if (res.ok) { const u = await res.json(); setUser(u); return; }
    } catch {}
  ...
  const apiFetch = async (path: string, opts: RequestInit = {}) => {
    ...
    if (!res.ok) { const e = await res.json().catch(() => ({ detail: 'Request failed' })); throw new Error(e.detail); }
    return res.json();
  };
